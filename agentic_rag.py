...
doesn't need a sub-graph for a loop that's bounded and doesn't need external
observability at each step. Keeps the top-level graph the same shape as the
reference tutorials it's adapted from.
"""
...
def grade_documents(question: str, documents: List[Document], llm) -> List[Document]:
...
def build_app(chunks: List[Document], llm=None):
    ...
    def node_route(state: AgentState) -> AgentState:
        return {"route": route_question(state["question"], llm)}

    def node_retrieve_simple(state: AgentState) -> AgentState:
        docs = hybrid_retriever.invoke(state["question"])
        return {"documents": docs}

    def node_retrieve_multi_hop(state: AgentState) -> AgentState:
    ...
    def node_complex(state: AgentState) -> AgentState:
        ...
        sub_questions = decompose_question(state["question"], llm)
        qa_pairs = []
        all_relevant_docs: List[Document] = []
        for sub_q in sub_questions:
            docs = hybrid_retriever.invoke(sub_q)
            relevant = grade_documents(sub_q, docs, llm)
            retries = 0
            current_q = sub_q
            while not relevant and retries < MAX_RETRIES_PER_SUBQUESTION:
                current_q = transform_query(current_q, llm)
                docs = hybrid_retriever.invoke(current_q)
                relevant = grade_documents(sub_q, docs, llm)
                retries += 1
